...
exports.changePassword = function (req, res) {
  ...
  .then(modifiedProfile => {
    ...
    .then(template => {
      var modifiedText = "Hi "+modifiedProfile[0].firstName+",\n\n"+template[0].text+"\n\n Username: "+req.body.username+"\n Password: "+req.body.password;
        var mailOptions = {
...
exports.editDefaultResume = (req, res) => {
  ...
  Validator.isValidCredentials(req)
  .then(result => { return ProfileManagementService.getProfileByAuthCredentials(req); })
  .then(profile => { return return ResumeManagementService.checkResume(req.params.resumeUuid, profile.uuid); })
  .then(data => { return ProfileManagementService.editDefaultResume(profile.uuid, req.params.resumeUuid); })
  .then(updatedProfile => { return res.status(200).send(updatedProfile); })
  .catch(err => {
    console.error('Err: %s', JSON.stringify(err));
    return res.status(err.code).send(err);
