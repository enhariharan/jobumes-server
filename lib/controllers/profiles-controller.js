...
exports.saveJobs = function (req, res) {
  ...
  console.log("request body: %s", JSON.stringify(req.body));
  var saveJob = {};
  saveJob.job = req.body.job;
  saveJob.status= req.body.status;

  console.log("saveJob uuid: "+saveJob.job+" , saveJob status:"+saveJob.status);

  Validator.isValidCredentials(req)
  .then(result =>{
    // console.log('result :: %j',result);
    return ProfileManagementService.saveJobs(credentials.name, saveJob);
  })
  .then(savedJobs => {
    console.info('in controller - updated jobs for the employee: ' + JSON.stringify(savedJobs.uuid));
    return res.sendStatus(200);
  })
  .catch(err => {
    console.error('Err: %s', JSON.stringify(err));
...
exports.getSavedJobs = (req, res) => {
  ...
  .then((verfiedProfileJobs) => {
    console.log('verfiedProfileJobs:: %j',verfiedProfileJobs);
    return new Promise(
      (resolve, reject) => {
        var i = 0;
        var savedJobs = verfiedProfileJobs.savedJobs;
        var m = savedJobs.length;
     console.log("jobs count: "+savedJobs.length);

    savedJobs.forEach(a => {
      console.log("verfiedProfileJobs: "+a.job);
        i++;
