...
exports.addNewUser = function (req, res) {
  ...
  .then((userAlreadyExists, userDTO) => {
    if (userAlreadyExists) {
      console.info('user %s %s (%s) already present', userDTO.firstName, userDTO.lastName, userDTO.email);
      return res.status(200).send(userDTO);
    } else {
      console.info('added new user: %s %s (%s)', userDTO.firstName, userDTO.lastName, userDTO.email);
