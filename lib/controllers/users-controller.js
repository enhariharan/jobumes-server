...
exports.addUser = function (req, res) {
  ...
  .then(user => {
    console.info('in controller - added new user: ' + JSON.stringify(user.username));
...
exports.addFbUser = function (req, res) {
  "use strict";
  ...
  UserManagementService.addFbUser(credentials, userdetails)
  .then(user => {
    console.info('in controller - added new user: ' + JSON.stringify(user.username));
    return res.status(201).send(user);
  }).catch(err => {
