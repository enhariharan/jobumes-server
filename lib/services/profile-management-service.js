...
var updateProfileImage = (profile, imageFile) => {
  ...
  return new Promise(
    (resolve, reject) => {
      console.info('profile: %j', profile);
      console.info('imageFile: %j', imageFile);
      // console.log("imageFileName::"+imageFile.filename);
      ...
      // console.log('last modified:: %j',lastModified);
      ...
      image.file = new Buffer(data).toString('base64');
      profile.image = image;

      // console.log('image:: %j',image);
      var query = {"uuid": profile.uuid};
      var update = {$set:{"lastModified": lastModified, "image": image}};
      Profile.findOneAndUpdate(query, update).exec()
      .then(updatedProfile => {
        console.log(" ");
        console.log("");
        console.log('updatedProfile: : %j',updatedProfile);
        ...
        console.log("");
        console.log("updatedProfileDto: %j", updatedProfileDto);
...
module.exports = {getProfileByUuid, getProfileByUser, getProfileByUsername, getProfileByAuthCredentials,
  addProfile, changePassword, updateProfileImage};
