...
var saveJobs = (username, bookmarkDetails) => {
  ...
  return new Promise(
    (resolve, reject) => {
      var model = {};
      model.uuid = Utilities.getUuid();
      model.created = {};
      model.created.timestamp = Utilities.getTimestamp();

      Profile.findOne({'login.username': username}, {'uuid': 1}).exec()
      .then(profile => {
        if (!profile || profile === undefined) throw(Errors.userProfileCouldNotBeFound);
        model.created.by = profile.uuid;
        model.profile = profile.uuid;
        return Job.findOne({uuid: bookmarkDetails.job}, {uuid: 1});
      })
      .then(job => {
        if (!job || job === undefined) throw(Errors.jobNotFound);
        model.job = bookmarkDetails.job;
        model.status = bookmarkDetails.status;
        return new JobProfile(model).save()
      })
