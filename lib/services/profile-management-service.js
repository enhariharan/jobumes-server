...
var _createNewJobProfile = (profile, job, status) => {
  return new Promise(
    (resolve, reject) => {
      ...
      if (status === 'saved') { model.savedOn = timestamp; }
      else if (status === 'applied') { model.appliedOn = timestamp; }
...
var _modifyExistingJobProfile = (jp, status) => {
  return new Promise(
    (resolve, reject) => {
      ...
      if (status === 'saved' && (jp.savedOn === null || jp.savedOn === undefined)) {
        update = {$set: {savedOn: timestamp}};
        updateRequired = true;
      }
      else if ((status === 'applied')  && (jp.appliedOn === null || jp.appliedOn === undefined)) {
        update = {$set: {appliedOn: timestamp}};
        updateRequired = true;
      }
