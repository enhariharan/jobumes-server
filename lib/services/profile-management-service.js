...
var _createNewJobProfile = (profile, job, status) => {
  return new Promise(
    (resolve, reject) => {
      ...
      if (status === 'saved') { model.saved = timestamp; }
      else if (status === 'applied') { model.applied = timestamp; }
...
var _modifyExistingJobProfile = (jp, status) => {
  return new Promise(
    (resolve, reject) => {
      var timestamp = Utilities.getTimestamp();
      var updateRequired = false;
      var condition = {profile:jp.profile, job:jp.job};
      var update = null;
      var options = {new: true}; // Need updated document immediately after findOneAndUpdate()

      if (status === 'saved' && (jp.saved === null || jp.saved === undefined)) {
        update = {$set: {saved: timestamp}};
        updateRequired = true;
      }
      else if ((status === 'applied')  && (jp.applied === null || jp.applied === undefined)) {
        update = {$set: {applied: timestamp}};
        updateRequired = true;
      }
...
//   then the nearby timestamp is saved into job-profile-model.saved.
// If the "status" field in request-body JSON is "applied"
//   then the nearby timestamp is saved into job-profile-model.applied.
var saveJobs = (username, bookmarkDetails) => {
  ...
  return new Promise(
    (resolve, reject) => {
      ...
      .then(j => {
        if (!j || j === undefined) throw(Errors.jobNotFound);
        job = j;
      ...
      .then(jp => {
        if (!jp || jp === undefined) { return _createNewJobProfile(profile, job, status); }
        else { return _modifyExistingJobProfile(jp, status); }
      })
      .then(result => { resolve(result); })
      .catch(err => {
        if (err.code === undefined) { reject({code: '500', reason: err}); }
