...
var _getJobApplicants = (jobs) => {
  ...
  return new Promise(
    (resolve, reject) => {
      ...
      jobs.forEach(j => {
        i++;
        // console.log('jobs:: %j',j);
        _getApplicantsDetails(j.uuid)
        .then(applicants => {
          console.log('aplicatns %j',applicants);
          var applicantsDetails = {};
          applicantsDetails.jobDetails = j;
          applicantsDetails.jobProfiles = applicants;
          applicantsDTO.push(applicantsDetails);
          // console.log('applicant :: %j',applicants);
          if(i === m){
