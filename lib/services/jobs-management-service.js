...
var JobApplicant = require('../models/job-applicants-model').JobApplicant;
...
var _parseJobSoap = (jobDtoToSave, metadata) => {
  return new Promise(
    (resolve, reject) => {
      soap.createClient(Configuration.jdParser.serviceUrl, (err, client) => {
        ...
        // console.info('\nclient: %s', util.inspect(client));
          ...
          client.ParseJD(options, (err, res) => {
            ...
            var xml = res.return;
            if (/error/.test(xml) === true) reject(xml);
            else resolve(xml);
  ...
  return new Promise(
    (resolve, reject) => {
      xml2jsParser.parseString(xml, (err, json) => {
        if (err && err !== undefined) reject(err);
        else resolve(json);
      });
  });
};
  ...
  return new Promise(
    (resolve, reject) => {
      ...
      .then(jdFileData => {
        ...
        return _parseJobSoap(jobDtoToSave, uploadedJDFileMetadata);
      })
      .then(parsedJobXml => {
        if (/error/.test(parsedJobXml) === true) throw(parsedJobXml);
...
exports.getApplicantsByJob = (jobUuid) => {
  return new Promise(
    (resolve, reject) => {
      JobApplicant.findOne({job:jobUuid}).exec()
