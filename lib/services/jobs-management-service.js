       ...
       .then(job => {
        ...
        //  console.log('job :: %j',job);
         if(!job.updateParsedJson || job.updateParsedJson === undefined){
           ...
           if(!job.updateParsedJson.JobData.IndustryType || job.updateParsedJson.JobData.IndustryType === undefined || job.updateParsedJson.JobData.IndustryType == ""){
             jobToBeSent.updateParsedJson.JobData.IndustryTypeName = "";

             if(!job.updateParsedJson.JobData.FunctionalArea || job.updateParsedJson.JobData.FunctionalArea === undefined || job.updateParsedJson.JobData.FunctionalArea == ""){
               jobToBeSent.updateParsedJson.JobData.FunctionalAreaName = "";
               resolve(jobToBeSent);
             }else{
               FunctionalAreaManagementService.getFunctionalAreaByUuid(job.updateParsedJson.JobData.FunctionalArea)
              .then(functionalArea => {
                jobToBeSent.updateParsedJson.JobData.FunctionalAreaName = functionalArea.name;
                console.log(jobToBeSent);
                resolve(jobToBeSent);
              })
...
exports.searchKeyword = (job) => {
  return new Promise(
    (resolve, reject) => {
      ...
      var searchElasticKeyword  = {
                                    "_source":["name","parsedJson.ResumeParserData.JobProfile"],
                                    "query": {
                                       "match" : {
                                         "parsedJson.ResumeParserData.JobProfile" : keyword
                                       }
                                       }
                                     };
       console.log('searchElasticKeyword :: %j',searchElasticKeyword);
