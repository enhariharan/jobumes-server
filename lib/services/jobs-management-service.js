      ...
      Job.findOne({uuid: jobUuid},{"uuid":1,"name":1,"status":1,"parsedJson":1,"updateParsedJson":1}).exec()
       .then(job => {
         ...
         if(!job.updateParsedJson || job.updateParsedJson === undefined){
           ...
           if(!updateParsedJson.JobData.IndustryType || updateParsedJson.JobData.IndustryType === undefined){resolve(jobToBeSent); }
           else{
             IndustryTypeManagementService.getIndustryTypeByUuid(updateParsedJson.JobData.IndustryType)
             .then(industryType => {
               jobToBeSent.updateParsedJson.JobData.IndustryTypeName = industryType.name;
               if(!updateParsedJson.JobData.FunctionalArea || updateParsedJson.JobData.FunctionalArea === undefined){resolve(jobToBeSent); }
               else{
                 FunctionalAreaManagementService.getFunctionalAreaByUuid(updateParsedJson.JobData.FunctionalArea)
                 .then(functionalArea => {
