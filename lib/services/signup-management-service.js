...
var _validate = (newUserDetails) => {
  return new Promise(
    (resolve, reject) => {
      ...
      .then(user => {
        if (user && user !== undefined) { resolve({userAlreadyExists: true, validatedUserDetails: newUserDetails}); }
        return(Role.findOne({uuid: newUserDetails.role}).exec());
      })
      .then(role => {
        if (!role || role === undefined) { throw(Errors.invalidRoleUuid); }
        resolve({userAlreadyExists: false, validatedUserDetails: newUserDetails});
      })
      .catch(err => {
...
exports.addNewUser = (newUserDetails) => {
  return new Promise(
    (resolve, reject) => {
      ...
      .then(validateduser => {
        if (validateduser.userAlreadyExists) {
          userAlreadyExistsFlag = true;
          // user with given details already exists. So return with user details.
          return(ProfileManagementService.getProfileByUsername(validateduser.validatedUserDetails.username));
        } else {
          // add a new user
      ...
      .then(profile => { resolve({userAlreadyExists: userAlreadyExistsFlag, profile: profile}); })
      .catch(err => {
        if (err.code === undefined) { reject({code: '500', reason: err}); }
        reject(err);
      });
