...
exports.getResumeByUuid = (resumeUuid) => {
  return new Promise(
    (resolve, reject) => {
      Resume.find({uuid: resumeUuid},{"uuid":1,"timestamp":1,"name":1,"parsedJson":1}).exec()
      .then(resumes => {
        resolve(resumes);
      })
      .catch(err => {
  ...
  return new Promise(
    (resolve, reject) => {
      console.log("searchstring",searchstring.search);
        var query ={'parsedJson.ResumeParserData.Skills': {'$regex': 'Cost Management'}};
        var fetchFeilds = {"uuid":1,"parsedJson.ResumeParserData.HtmlResume":0,"name":1,"type":1,"status":1};
