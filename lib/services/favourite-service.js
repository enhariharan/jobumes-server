...
var _validateExists = (recruiterUuid,fav) => {
  return new Promise(
    (resolve, reject) => {
      ...
      .then(user => {
        if (!user.status || user.status === undefined) { resolve({exists: false, status: "new" }); }
  ...
  return new Promise(
    (resolve, reject) => {
      ...
      .then(validatedUser => {
        ...
        if(validatedUser.exists === true && validatedUser.status === 'favourite'){ reject(Errors.addedFavourite); }
        if(validatedUser.exists === true && validatedUser.status === 'non-favourite'){
          ...
          var update = {$set:{"status":"favourite","lastModified":lastModified}};
...
exports.listFavourites = (profileUuid) => {
  return new Promise(
    (resolve, reject) => {
      Favourite.find({"recruiter":profileUuid,"status":"favourite"},{"uuid":1,"recruiter":1,"status":1,"jobseeker":1,"timestamp":1,"lastModified":1})
