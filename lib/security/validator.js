...
var isValidCredentials = (req) => {
  return new Promise(
    (resolve, reject) => {
      ...
      .then(u => {
        if (!u || u === undefined
           || credentials.name.toLowerCase().localeCompare(u.login.username.toLowerCase())
           || credentials.pass.localeCompare(u.login.password)) throw(Errors.invalidCredentials);
  ...
  return new Promise(
    (resolve, reject) => {
      ...
      .then(result => {
        ...
        return ProfileManagementService.getProfileByCredentials(credentials);
      })
      .then(user => {
