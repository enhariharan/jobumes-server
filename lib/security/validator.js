...
var isValidCredentials = (req) => {
  return new Promise(
    (resolve, reject) => {
      ...
      // "userName" must be set to the email use at the social login
      if (req.headers.source && req.headers.source !== undefined &&
          req.headers.userName && req.headers.userName !== undefined) {
        Profile.find({}, {uuid:1, login:1, socialProfiles:1}).exec()
        .then(profiles => {
          var found = false;
          profiles.forEach(p => {
            if (p.login.username === req.headers.userName) {
              p.socialProfiles.forEach(s => {
                if (s.socialNetworkName === req.headers.source && s.email === req.headers.userName) {
                    found = true;
